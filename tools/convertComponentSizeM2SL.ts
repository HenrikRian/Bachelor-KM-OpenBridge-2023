...
function parse(name: string) {
  ...
  const fileLarge = file.replace(/Medium/g, 'Large')
    .replace(/medium/g, 'large')
    .replace(/\d+\..\d*/g, substring => (substring === '.' ? '.' : parseFloat(substring) * 2).toString())

  const mediumName = name.replace('medium', 'large')
  fs.writeFileSync(mediumName, fileLarge)

  const fileSmall = file.replace(/Medium/g, 'Small')
