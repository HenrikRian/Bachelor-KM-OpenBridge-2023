...
export async function getFigmaSvg(fileId: string, elementId: string): Promise<string> {
    const imageUrlData = await fetch(`${baseUrl}/v1/images/${fileId}?ids=${elementId}&format=svg&svg_include_id=true`, header);
