...
<script>
export default {
  name: "PrimaryTicksMedium",
  props: {
    rotate: {type: Number, required: false, default: 0},
    showPointer: {type: Boolean, required: false, default: true},
    showDeg: {type: Number, required: true},
  ...
  computed: {
    show() {
      return (deg) => {
        return !(this.endClipDeg > deg + 360 || (this.startClipDeg < deg && this.endClipDeg > deg));
      }
    }
  }
}
